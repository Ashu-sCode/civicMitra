...
import { Camera, MapPin, Upload, Check, Wifi, WifiOff, Phone, User, MessageSquare, Folder, ChevronDown } from 'lucide-react';
...
const ReportPage = () => {
  ...
  const handleSubmit = async () => {
    if (!formData.fullName || !formData.mobileNumber || !formData.category || !formData.description) {
    ...
    try {
      ...
      const reportId = await saveToIndexedDB(formData);
  ...
  if (isSubmitted) {
    return (
      <div className={`min-h-screen transition-colors duration-300 ${isDarkMode ? 'bg-gray-900' : 'bg-gradient-to-br from-blue-50 to-gray-100'}`}>
        <div className="max-w-md mx-auto px-4 py-8 md:max-w-2xl lg:py-12">
          <div className={`rounded-2xl p-8 text-center shadow-xl transition-colors duration-300 ${isDarkMode ? 'bg-gray-800 border border-gray-700' : 'bg-white border border-gray-200'}`}>
    ...
    <div className={`min-h-screen transition-colors duration-300 ${isDarkMode ? 'bg-gray-900' : 'bg-gradient-to-br from-blue-50 to-gray-100'}`}>
        ...
        <div className="max-w-md mx-auto px-4 py-3 md:max-w-2xl">
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              <div className={`w-3 h-3 rounded-full mr-2 ${isOnline ? 'bg-green-500' : 'bg-orange-500'} animate-pulse`}></div>
      ...
      <div className="max-w-md mx-auto px-4 py-6 md:max-w-2xl lg:py-8">
        ...
        <div className="space-y-6">
          {/* Complainant Details */}
          <div className={`rounded-2xl p-6 shadow-lg transition-colors duration-300 ${isDarkMode ? 'bg-gray-800 border border-gray-700' : 'bg-white border border-gray-200'}`}>
          ...
          <div className={`rounded-2xl p-6 shadow-lg transition-colors duration-300 ${isDarkMode ? 'bg-gray-800 border border-gray-700' : 'bg-white border border-gray-200'}`}>
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="relative">
                  <Folder className={`absolute left-3 top-1/2 transform -translate-y-1/2 w-5 h-5 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`} />
                  <ChevronDown className={`absolute right-3 top-1/2 transform -translate-y-1/2 w-5 h-5 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`} />
                  <select
          ...
          {/* Offline Notice */}
          {!isOnline && (
            <div className={`text-center p-4 rounded-xl border ${isDarkMode ? 'bg-orange-900/20 border-orange-700 text-orange-300' : 'bg-orange-50 border-orange-200 text-orange-800'}`}>
